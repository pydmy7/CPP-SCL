...
namespace scl {
    ...
    template<typename T = int, const char op = 'S'>
    class Fenwick {
    private:
        const int n;
        std::vector<T> tr;
    ...
    public:
        explicit Fenwick(const int n) : n(n), tr(n, 0) {}
        ...
        void add(const int idx, const T val) {
            assert(0 <= idx && idx <= n);
        ...
        int kth(const T x) {  // min pos s.t. sum[0, pos] >= x
            assert(x >= 0);
